...
def get_scatter_point(entered_site):
    modified_df = spacex_df
    if entered_site == 'ALL':
        fig = px.scatter(modified_df,x='Payload Mass (kg)',y='class', color='Booster Version Category', title = 'Correlation between Payload and Success for all Sites')
        return fig
    else:
        modified_df = spacex_df[spacex_df['Launch Site']==entered_site].groupby(['Launch Site','class']).size().reset_index(name='class count')
        title = f"Correlation between Payload and Success for Site {entered_site}"
        fig = px.scatter(modified_df,x='Payload Mass (kg)',y='class', color='Booster Version Category', title = title)
        return fig
